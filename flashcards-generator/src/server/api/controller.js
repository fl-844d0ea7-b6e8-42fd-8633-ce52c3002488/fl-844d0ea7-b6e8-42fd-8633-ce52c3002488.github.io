...
export const listTopics = async (request, response) => {
  ...
  logInfo("Received request to LIST TOPICS")

  logInfo(`Making requests to: ${API_HOSTNAME}/api/listTopics`)

  axios
    .get(`${API_HOSTNAME}/api/listTopics`, config)
    .then((resp) => response.status(200).send(resp.data))
    .catch((error) => {
      console.error('Something went wrong', error && error.Error)
      return response.sendStatus(503)
    })
}
